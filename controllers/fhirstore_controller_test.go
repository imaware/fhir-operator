...
func TestExportFhirStore(t *testing.T) {
	...
	fhirStore := &v1alpha1.FhirStore{
		...
		Spec: v1alpha1.FhirStoreSpec{
			FhirStoreID: FHIRSTORE_ID,
			DatasetID:   DATASET_ID_IT,
			ExportOptions: v1alpha1.FhirStoreSpecExportOptions{
				EnableExports: true,
				Location:      "gs://integration-test-fhir-op/test",
				Frequency:     "1m",
			},
		},
	...
	_, err = exportFhirStoreLoop(fhirStore)
	if err != nil {
		fmt.Printf("Failed to export fhirstore %v", err.Error())
	}
